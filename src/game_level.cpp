...
void GameLevel::init(std::vector<std::vector<unsigned int>> tileData,
	...
	for (unsigned int y = 0; y < height; ++y) {
		for (unsigned int x = 0; x < width; ++x) {
			...
			} else if (tileData[y][x] == 2)  // cheese; now determine its color
											 // based on level data
			{
				// if (tileData[y][x] == 2)
				...
				glm::vec3 color = glm::vec3(1.0f);  // enemy

				glm::vec2 pos(unit_width * x, unit_height * y);
				glm::vec2 size(unit_width, unit_height);
				GameObject obj(pos, size, ResourceManager::GetTexture("enemy"),
							   glm::vec3(1.0f));
				obj.IsSolid = true;
				obj.IsEnemy = true;
				obj.IsFinish = false;
				...
				glm::vec3 color = glm::vec3(1.0f);  // finish

				glm::vec2 pos(unit_width * x, unit_height * y);
				glm::vec2 size(unit_width, unit_height);
				GameObject obj(pos, size, ResourceManager::GetTexture("finish"),
							   glm::vec3(1.0f));
				obj.IsFinish = true;
				obj.IsEnemy = false;
				obj.IsSolid = false;
